...
public class Juego {
    ...
    public Juego(int modoJuego, JuegoController gameController) {
        this.elegirModoDeJuego = modoJuego;
        this.gameController = gameController;
    }

    void clickButton(Button button) {
        this.buttonClick = button;
        if (elegirModoDeJuego == 1) {
            playerVsPc();
        } else if (elegirModoDeJuego == 2) {
            pcVsPc();
        } else if (elegirModoDeJuego == 3) {
            jugarVSjugador();
        }

        if (botonesPulsados.size() >= buttons.size() && posicionesGanadoras == null) {
            gameController.reiniciarJuego();
            if (elegirModoDeJuego != 2) {
                JuegoController.playerList.get(JuegoController.playerList.size() - 1).añadirResultado(1);
            }
        }
    }
